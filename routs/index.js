...
router.get('/', (req, res) => {
   Promise.all([SkillCat.find({}), Project.find({}), Experience.find({})])
      .then(dbEntries => {
         let { organizeSiteDataFromPriority } = utilities;

         let skillCat = dbEntries[0];
         let projects = dbEntries[1];
         let experiences = organizeSiteDataFromPriority(dbEntries[2]);
         res.render('index', {
            skillCats: skillCat,
            projects: projects,
      ...
      .catch(err => {
         res.status(500).send(
            `failed to load index page becaiuse ${err.message}`
         );
      });
});
...
router.get('/download', (req, res) => {
   res.download(
      __dirname + '/../downloads/ResumeAnthonyCavuotifullstackwebdeveloper.pdf'
   );
});
